...
import { ArrowLeft, Award, User, FileText, Calendar, Clock, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function SupervisionReadOnly({ projectCode, projectTitle }: SupervisionReadOnlyProps) {
    const navigate = useNavigate();
    const [expandedDelivery, setExpandedDelivery] = useState<number | null>(null);
    const currentStep = 3;

    const displayTitle = projectTitle ?? MOCK_PROJECT.title;
    const displayCode = projectCode ?? MOCK_PROJECT.code;
    ...
    return (
        <div className="flex flex-col gap-6">
            <PageHeader
                eyebrow="Supervisión"
                title={displayTitle}
                subtitle={`${displayCode} · ${MOCK_PROJECT.students}`}
                actions={
                    <button
                        onClick={() => navigate(-1)}
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="flex items-center gap-3 rounded-lg border border-[#e5e5e5] bg-[#fafaf9] p-3.5">
                        ...
                        <div>
                            ...
                            <p className="text-sm font-semibold text-[#1c1917]">{MOCK_PROJECT.students}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3 rounded-lg border border-[#e5e5e5] bg-[#fafaf9] p-3.5">
